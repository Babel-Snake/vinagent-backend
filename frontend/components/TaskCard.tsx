...
function TaskSection({ title, summary, count, children }: TaskSectionProps) {
    return (
...
type TaskPanelKey = 'work' | 'customer' | 'conversation' | 'files' | 'outcome' | 'activity';
...
export default function TaskCard({
    ...
    function selectPanel(panel: TaskPanelKey) {
        ...
        if ((panel === 'conversation' || panel === 'activity') && !historyOpen) {
            setHistoryOpen(true);
            loadHistory();
        }
    }
    ...
    const summarizedTaskText = typeof requestPayload?.summary === 'string'
        ? requestPayload.summary.trim()
        : '';
    const originalRequestText = typeof requestPayload?.originalText === 'string'
        ? requestPayload.originalText.trim()
        : '';
    const requestSummary = summarizedTaskText || originalRequestText || String(task.notes || 'Original request and task context');
    const showOriginalRequestText = Boolean(originalRequestText && originalRequestText !== requestSummary);
    ...
    const panelItems: Array<{ key: TaskPanelKey; label: string; detail: string }> = [
        { key: 'work', label: 'Work', detail: `${orderedTaskSteps.length} step${orderedTaskSteps.length === 1 ? '' : 's'}` },
        { key: 'customer', label: 'Customer', detail: task.Member ? 'Linked record' : (manualIntake ? formatIdentityResolutionStatus(manualIntake.identityResolutionStatus) : 'Requester') },
        { key: 'conversation', label: 'Conversation', detail: `${safeTaskMessages.length} message${safeTaskMessages.length === 1 ? '' : 's'}` },
        { key: 'files', label: 'Files & Links', detail: `${linkedNotices.length} notice${linkedNotices.length === 1 ? '' : 's'}` },
        { key: 'outcome', label: 'Outcome', detail: humanize(task.status) },
        { key: 'activity', label: 'Activity', detail: `${safeTaskActions.length} item${safeTaskActions.length === 1 ? '' : 's'}` }
    ];
        ...
        >
            <div className="min-w-0 w-full space-y-4">
                ...
                <section className="rounded-lg border border-[#dfe6da] bg-white p-4">
                    <div className="mb-2 text-xs font-bold uppercase tracking-wider text-[#344039]">Initial issue</div>
                    <div className="whitespace-pre-wrap text-sm leading-6 text-[#344039]">
                        {requestSummary}
                    </div>
                    {showOriginalRequestText && (
                        <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3">
                            <div className="mb-1 text-[11px] font-bold uppercase tracking-wider text-slate-500">Original input</div>
                ...
                <nav className="lg:hidden" aria-label="Task sections">
                            ...
                            return (
                                <button
                                    ...
                                    className={`shrink-0 rounded-md border px-3 py-2 text-left transition ${
                                        selected
                                            ? 'border-[#0f766e] bg-[#e8f5ef] text-[#0f4f43]'
                    ...
                    <div className="space-y-4">
                        ...
                        >
                            <div className="grid grid-cols-1 gap-3 text-sm md:grid-cols-3">
                                <div>
                                    <div className="mb-1 text-[11px] font-bold uppercase tracking-wider text-slate-500">Name</div>
                                    <div className="text-slate-900">{requesterName}</div>
                                </div>
                                <div>
                                    <div className="mb-1 text-[11px] font-bold uppercase tracking-wider text-slate-500">Email</div>
                                    <div className="break-words text-slate-900">{task.Member?.email || manualIntake?.requesterEmail || 'No email captured'}</div>
                                </div>
                                <div>
                                    <div className="mb-1 text-[11px] font-bold uppercase tracking-wider text-slate-500">Phone</div>
                                    <div className="text-slate-900">{task.Member?.phone || manualIntake?.requesterPhone || 'No phone captured'}</div>
                                </div>
                            </div>
                            <div className="mt-3 text-xs text-slate-600">
                                {task.Member
                                    ...
                                    : manualIntake
                                        ? `Identity status: ${formatIdentityResolutionStatus(manualIntake.identityResolutionStatus)}`
                ...
                {activePanel === 'outcome' && (
                    <div className="space-y-4">
                        <TaskSection
                            title="Case Controls"
                            summary={`${humanize(task.status)} / ${assigneeName}`}
                        >
                                ...
                                return (
                                    ...
                                    <div className="mt-3 flex flex-wrap gap-2">
                                        ...
                                        <button
                                            type="button"
                                            disabled={updating || Boolean(stepLockReason)}
                                            onClick={() => handleStepUpdate(step.id, { status: 'SKIPPED', blockedReason: null, completionNotes: null, waitingOn: 'NONE' })}
                                            className="px-2.5 py-1.5 rounded bg-amber-600 text-white text-xs font-bold hover:bg-amber-700 disabled:opacity-50"
                                        >
                                            Skip
                                        </button>
                                        <button
                ...
                {(activePanel === 'conversation' || activePanel === 'activity') && (
                    <div className="space-y-4">
                    <TaskSection
                        title={activePanel === 'conversation' ? 'Conversation' : 'Activity'}
                        summary={activePanel === 'conversation'
                            ? `${safeTaskMessages.length} message${safeTaskMessages.length === 1 ? '' : 's'} / notes`
                            : `${safeTaskActions.length} activity item${safeTaskActions.length === 1 ? '' : 's'}`}
                        count={activePanel === 'conversation' ? safeTaskMessages.length : safeTaskActions.length}
                    >
                    {activePanel === 'activity' && historyOpen && (
            ...
            <aside className="mt-5 space-y-3 lg:sticky lg:top-4 lg:mt-0">
                <nav className="hidden rounded-lg border border-[#dfe6da] bg-white p-3 lg:block" aria-label="Task sections">
                    <div className="mb-2 px-1 text-xs font-bold uppercase tracking-wider text-[#344039]">Sections</div>
                    <div className="space-y-1">
                        {panelItems.map(item => {
                            const selected = activePanel === item.key;
                            return (
